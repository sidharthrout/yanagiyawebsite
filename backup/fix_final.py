...
# 3. Fix Preloader to always show
old_preloader = '''  //  PRELOADER (Zen Reveal) 
...
new_preloader = '''  //  PRELOADER (Zen Reveal) 
  document.addEventListener('DOMContentLoaded', () => {
    const zp = document.getElementById('zen-preloader');
    if (zp) {
      // Force scroll to top on load to prevent weird scroll positions
      window.scrollTo(0,0);
...
# Just in case the \r\n vs \n is an issue, let's use regex
content = re.sub(
    r"//  PRELOADER \(Zen Reveal\) .*?\}\);\r?\n",
    new_preloader + '\n',
    content,
    flags=re.DOTALL
)

# 4 & 5. Add Trail zoom-scroll effect and fix Magome/Stars smooth exit

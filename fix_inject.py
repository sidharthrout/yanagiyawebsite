...
# Find the end of style block. In index.html, it's </style> on line ~790
...
# Find the closing body script tag and inject before it
# We'll replace the line switchCalendar(lodge, btn) end block with this
target_str = 'switchCalendar(lodge, btn) {\n    document.querySelectorAll('
if target_str in content:
    # Just inject right before the end of the script block
    content = content.replace('  }\n</script>', '  }\n\n' + ds_js + '\n</script>', 1)
else:
    # Fallback, find the last </script> by rfind
    idx = content.rfind('</script>')
    if idx != -1:
        content = content[:idx] + ds_js + '\n' + content[idx:]
